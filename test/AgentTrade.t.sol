...
contract AgentTradeTest is Test {
    ...
    function setUp() public {
        ...
        agentId1 = registry.registerAgent(
            "AlphaBot", "Momentum breakout on USDC/WMON", "Ride short-term momentum shifts", AGENT1_FEE_BPS
        );

        vm.prank(agent2Owner);
        agentId2 =
            registry.registerAgent("BetaBot", "Mean reversion USDC/WETH", "Capture spread compression", AGENT2_FEE_BPS);
    }
    ...
    function test_registerAgent_revert_alreadyRegistered() public {
        ...
        vm.prank(agent1Owner);
        vm.expectRevert(abi.encodeWithSelector(IAgentRegistry.AgentRegistry__AlreadyRegistered.selector, agent1Owner));
    ...
    function test_registerAgent_revert_feeTooHigh() public {
        ...
        vm.prank(newOwner);
        vm.expectRevert(abi.encodeWithSelector(IAgentRegistry.AgentRegistry__FeeTooHigh.selector, badFee, 2000));
    ...
    function test_deactivateAgent_revert_notOwner() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IAgentRegistry.AgentRegistry__NotAgentOwner.selector, alice, agentId1));
        registry.deactivateAgent(agentId1);
    }
    ...
    function test_deposit_revert_agentNotActive() public {
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(IDelegationVault.DelegationVault__AgentNotActive.selector, agentId1));
    ...
    function test_withdraw_success() public {
        ...
        assertEq(
            usdc.balanceOf(alice), aliceBefore + withdrawAmt - expectFee, "alice should receive net of platform fee"
        );
        assertEq(feeContract.accumulatedFees(), feesBefore + expectFee, "platform fee accumulated");
    }
    ...
    function test_distributeProfits_revert_notAgentOwner() public {
        ...
        usdc.approve(address(vault), profit);
        vm.expectRevert(
            abi.encodeWithSelector(IDelegationVault.DelegationVault__NotAgentOwner.selector, alice, agentId1)
        );
        vault.distributeProfits(agentId1, profit);
        vm.stopPrank();
    }
    ...
    function test_claimRewards_revert_noRewards() public {
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(IDelegationVault.DelegationVault__NoRewards.selector, alice, agentId1));
        vault.claimRewards(agentId1);
    }
    ...
    function test_updateFeePercent_revert_tooHigh() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IPlatformFee.PlatformFee__FeeTooHigh.selector, badFee, 1000));
    ...
    function test_reviewEligibility_onlyPastDelegators() public {
        ...
        vm.prank(charlie);
        vm.expectRevert(
            abi.encodeWithSelector(IAgentRegistry.AgentRegistry__NotEligibleReviewer.selector, charlie, agentId1)
        );
        ...
        vm.prank(charlie);
        vm.expectRevert(
            abi.encodeWithSelector(IAgentRegistry.AgentRegistry__AlreadyReviewed.selector, charlie, agentId1)
        );
